$.extend(jasmine, {
  ...
  listsRepository: function(element, repository, errors) {
    ...
    var formattedFinishedAt = repository.get('lastBuildFinishedAt') ? $.timeago.distanceInWords(repository.get('lastBuildFinishedAt')) : '-';
    var formattedDuration   = repository.get('lastBuildDuration') ? Utils.readableTime(repository.get('lastBuildDuration')) : '-';
    ...
    var expected = {
      ...
      '.duration':      { title: repository.get('lastBuildDuration'), text: formattedDuration },
      '.finished_at':   { title: repository.get('lastBuildFinishedAt'), text: formattedFinishedAt }
    };
...
beforeEach(function() {
  this.addMatchers({
    toListRepositories: function(repositories) {
      ...
      var result = true;
      $.each(repositories, function(ix, repository) {

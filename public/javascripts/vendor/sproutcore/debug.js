...
// When in debug mode, itâ€™s useful for our observer sets (which are used by
...
SC.addInvokeOnceLastDebuggingInfo = function() {
        ...
        while(--idx>=0) {
          ...
          if (log) {
            ...
            if (!originatingTargets) {
              ...
              // hashes or none.)
              SC.Logger.log("Invoking runloop-scheduled method %@ on %@, but we didnâ€™t capture information about who scheduled itâ€¦".fmt(mName, target));
            }
            else {
  ...
  // output in some browser's debuggers.  The â€œrecentâ€ stack is capped at 6
  // entries.
